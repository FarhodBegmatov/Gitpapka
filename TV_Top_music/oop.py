...
def musicMenu():
    print("             "+bcolors.OKBLUE+"\n\n🎧"+bcolors.ENDC+bcolors.WARNING+"              🏠 Home page "+bcolors.ENDC)
    print(bcolors.OKBLUE + """
        1. 🎼 List of songs
        2. 🥇 Rating of songs
...
def listOfSongs():
    ...
    for item in list:
        ...
        if count < 10:
            stri = "0"
        if count < 1000 and count>99:
            stri = "00"
        if count < 10000 and count>999:
            stri = "000"
        massiv = item.split("like")

        print("  "+stri+str(count) + bcolors.OKBLUE + ". 🎵 " + bcolors.ENDC + massiv[0])
...
def listOfSongs2(massiv):
    ...
    if selectsong in massiv:
        p = multiprocessing.Process(target=playsound, args=("musics/Jan A.P. Kaczmarek - Hachiko Runs Away.mp3",))
        p.start()
    ...
    f.write(id_story.readline()+"$$" + music_massiv[selectsong - 1] + "   " + str(date.today()) + "  " + now.strftime("%H:%M:%S") + "\n")
    f.close()
    id_story.close()
